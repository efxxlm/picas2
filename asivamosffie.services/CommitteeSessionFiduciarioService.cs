...
namespace asivamosffie.services
{
    ...
    */
    ...
    public class CommitteeSessionFiduciarioService : ICommitteeSessionFiduciarioService
    {
        ...
        private void GetLiberarTemasComiteTecnico(ComiteTecnico comiteTecnicoOld)
        {
            _context.Set<SesionComiteTema>()
                    .Where(s => s.ComiteTecnicoId == comiteTecnicoOld.ComiteTecnicoId)
        ...
        public async Task<ComiteTecnico> GetComiteTecnicoByComiteTecnicoId(int pComiteTecnicoId)
        {
            ...
            ComiteTecnico comiteTecnico = await _context.ComiteTecnico.Where(r => r.ComiteTecnicoId == pComiteTecnicoId)
            ...
            List<SesionComiteTema> ListSesionComiteTema = _context.SesionComiteTema.Where(r => r.ComiteTecnicoId == pComiteTecnicoId)
            ...
            {
                foreach (var SesionTemaVoto in SesionComiteTema.SesionTemaVoto)
                {
                    SesionTemaVoto.SesionTema = null;
                }
            ...
            foreach (var SesionComiteSolicitud in comiteTecnico.SesionComiteSolicitudComiteTecnicoFiduciario)
            {
                ...
                foreach (var SesionSolicitudVoto in SesionComiteSolicitud.SesionSolicitudVoto)
                {
                    SesionSolicitudVoto.SesionParticipante.SesionSolicitudVoto = null;
                }
            }
            ...
            foreach (SesionComiteSolicitud sesionComiteSolicitud in comiteTecnico.SesionComiteSolicitudComiteTecnicoFiduciario)
            {
                ...
                switch (sesionComiteSolicitud.TipoSolicitudCodigo)
                {
                    case ConstanCodigoTipoSolicitud.Contratacion:
                        ...
                        Contratacion contratacion = _context.Contratacion.Where(r => r.ContratacionId == sesionComiteSolicitud.SolicitudId)
        ...
        private async Task<string> ReemplazarDatosPlantillaActa(string strContenido, ComiteTecnico pComiteTecnico)
        {
            try
            {
                ...
                if (ListNovedadContractual.Count > 0)
                {
                    ListContratacionNovedadContractual = _context.NovedadContractual.AsNoTracking()
                                                                     .Where(r => ListNovedadContractual.Contains(r.NovedadContractualId))
                ...
                foreach (SesionComiteSolicitud SesionComiteSolicitud in pComiteTecnico.SesionComiteSolicitudComiteTecnicoFiduciario.OrderByDescending(r => r.TipoSolicitudCodigo))
                {
                    ...
                    switch (SesionComiteSolicitud.TipoSolicitudCodigo)
                    {
                        ...
                        case ConstanCodigoTipoSolicitud.Novedad_Contractual:
                            ...
                            NovedadContractual novedad = ListContratacionNovedadContractual.Where(r => r.NovedadContractualId == SesionComiteSolicitud.SolicitudId).FirstOrDefault();
                ...
                foreach (var Tema in pComiteTecnico.SesionComiteTema.Where(r => r.EsProposicionesVarios == null).ToList())
                {
                    ...
                    foreach (Dominio placeholderDominio in placeholders)
                    {
                        switch (placeholderDominio.Codigo)
                        {
                            ...
                            case ConstanCodigoVariablesPlaceHolders.RESULTADO_VOTACION:
                                string TextoResultadoVotacion = String.Empty;
                                int cantidadAprobado = Tema.SesionTemaVoto.Count(r => r.Eliminado != true && r.EsAprobado.Value == true);
                                int cantidadNoAprobo = Tema.SesionTemaVoto.Count(r => r.Eliminado != true && r.EsAprobado.Value != true);
                    ...
                    foreach (Dominio placeholderDominio in placeholders)
                    {
                        switch (placeholderDominio.Codigo)
                        {
                            ...
                            case ConstanCodigoVariablesPlaceHolders.DECISIONES_SOLICITUD:
                                ...
                                string strRequiereVotacion = "";
                                int cantidadAprobadas = Tema.SesionTemaVoto.Count(r => r.Eliminado != true && r.EsAprobado.Value == true);
                                int cantidadNoAprobadas = Tema.SesionTemaVoto.Count(r => r.Eliminado != true && r.EsAprobado.Value != true);
                ...
                foreach (var scst in pComiteTecnico.SesionComiteSolicitudComiteTecnicoFiduciario)
                {
                    switch (scst.TipoSolicitudCodigo)
                    {
                        ...
                        case ConstanCodigoTipoSolicitud.Novedad_Contractual:
                            ...
                            RegistrosFichaNovedadContractual = await _IRegisterSessionTechnicalCommitteeService.ReemplazarDatosPlantillaNovedadContractual

                                (
                                RegistrosFichaNovedadContractual,
                                ListContratacionNovedadContractual.Where(r => r.NovedadContractualId == scst.SolicitudId).FirstOrDefault(),
                                ListPlantillas,
                                ListaParametricas,
                                localizacions,
        ...
        public async Task<byte[]> GetPlantillaActaIdComite(int ComiteId)
        {
            ...
            ComiteTecnico comiteTecnico = await _context.ComiteTecnico.AsNoTracking()
            ...
            foreach (var SesionComiteSolicitudComiteTecnicoFiduciario in comiteTecnico.SesionComiteSolicitudComiteTecnicoFiduciario)
            {
                SesionComiteSolicitudComiteTecnicoFiduciario.SesionSolicitudVoto =
                    _context.SesionSolicitudVoto.Where(r => r.SesionComiteSolicitudId == SesionComiteSolicitudComiteTecnicoFiduciario.SesionComiteSolicitudId
                                                        && r.Eliminado != true)
                                                .ToList();
            }
            ...
            //  Plantilla.Contenido =await _IRegisterSessionTechnicalCommitteeService.ReemplazarDatosPlantillaActa(Plantilla.Contenido, comiteTecnico);
