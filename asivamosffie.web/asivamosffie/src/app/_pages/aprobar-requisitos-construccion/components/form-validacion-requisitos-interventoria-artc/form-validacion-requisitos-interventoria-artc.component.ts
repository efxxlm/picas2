          ...
          this.faseUnoConstruccionService.getContratoByContratoId( this.activatedRoute.snapshot.params.id )
            .subscribe( response => {
              ...
              for ( let contratacionProyecto of this.contrato.contratacion.contratacionProyecto ) {
                ...
                for ( let perfil of contratacionProyecto.proyecto.contratoConstruccion[0].construccionPerfil ) {
                  const observacionTipo3 = [];
                  for ( let observacion of perfil.construccionPerfilObservacion ) {
                  ...
                  if ( observacionTipo3.length > 0 ) {
                    if ( observacionTipo3[ observacionTipo3.length -1 ].esSupervision !== undefined && observacionTipo3[ observacionTipo3.length -1 ].esSupervision === false ) {
                ...
                if ( semaforoCompleto > 0 && semaforoCompleto === contratacionProyecto.proyecto.contratoConstruccion[0].construccionPerfil.length )
                  contratacionProyecto[ 'estadoSemaforoContratacion' ] = 'completo';
                if ( semaforoSinDiligenciar > 0 && semaforoSinDiligenciar === contratacionProyecto.proyecto.contratoConstruccion[0].construccionPerfil.length ) 
                  contratacionProyecto[ 'estadoSemaforoContratacion' ] = 'sin-diligenciar';
                if ( semaforoEnProceso > 0 && semaforoEnProceso < contratacionProyecto.proyecto.contratoConstruccion[0].construccionPerfil.length ) 
                  contratacionProyecto[ 'estadoSemaforoContratacion' ] = 'en-proceso';
                if ( semaforoCompleto > 0 && semaforoSinDiligenciar > 0 && semaforoSinDiligenciar + semaforoCompleto === contratacionProyecto.proyecto.contratoConstruccion[0].construccionPerfil.length )
                  contratacionProyecto[ 'estadoSemaforoContratacion' ] = 'en-proceso';
              };
            } );
        }
