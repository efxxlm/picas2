...
export class FormTerceroCausacionComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.balanceFinanciero = await this.balanceSvc
      .getBalanceFinanciero(this.activatedRoute.snapshot.params.id)
      .toPromise();
    ...
    if (this.balanceFinanciero.balanceFinancieroTraslado !== undefined) {
      if (this.balanceFinanciero.balanceFinancieroTraslado.length > 0) {
        const traslado = this.balanceFinanciero.balanceFinancieroTraslado.find(
    ...
    for (const solicitudPagoFase of this.solicitudPago.solicitudPagoRegistrarSolicitudPago[0].solicitudPagoFase) {
      ...
      const criterioFormaPago = await this.registrarPagosSvc
        .getCriterioByFormaPagoCodigo(
          solicitudPagoFase.esPreconstruccion === true
            ? this.fasePreConstruccionFormaPagoCodigo.fasePreConstruccionFormaPagoCodigo
            : this.fasePreConstruccionFormaPagoCodigo.faseConstruccionFormaPagoCodigo
        )
        .toPromise();
      ...
      for (const criterioValue of solicitudPagoFaseCriterio) {
        ...
        if (criterioFind !== undefined) {
          ...
          const conceptosDePago = await this.registrarPagosSvc.getConceptoPagoCriterioCodigoByTipoPagoCodigo(
            tipoPago.codigo
          );
      ...
      for (const criterio of listCriterios) {
        ...
        if (this.ordenGiroDetalleTerceroCausacion !== undefined) {
          const terceroCausacion = this.ordenGiroDetalleTerceroCausacion.find(
            tercero =>
              tercero.conceptoPagoCriterio === criterio.tipoCriterioCodigo &&
              tercero.esPreconstruccion === solicitudPagoFase.esPreconstruccion
          );
          ...
          if (terceroCausacion !== undefined) {
            ...
            if (terceroCausacion.ordenGiroDetalleTerceroCausacionDescuento.length > 0) {
              for (const descuento of listDescuento) {
                const ordenGiroDetalleTerceroCausacionDescuento: any[] =
                  terceroCausacion.ordenGiroDetalleTerceroCausacionDescuento.filter(
                    ordenGiroDetalleTerceroCausacionDescuento =>
                      ordenGiroDetalleTerceroCausacionDescuento.tipoDescuentoCodigo === descuento.codigo
                  );
                ...
                if (ordenGiroDetalleTerceroCausacionDescuento.length > 0) {
                  for (const terceroCausacionDescuento of ordenGiroDetalleTerceroCausacionDescuento) {
                    ...
                    if (this.balanceFinancieroTrasladoValor !== undefined) {
                      if (this.balanceFinancieroTrasladoValor.length > 0) {
                        const balanceFinancieroTrasladoValor = this.balanceFinancieroTrasladoValor.find(
                          balanceFinancieroTrasladoValor =>
                            balanceFinancieroTrasladoValor.ordenGiroDetalleTerceroCausacionDescuentoId ===
                              terceroCausacionDescuento.ordenGiroDetalleTerceroCausacionDescuentoId &&
                            balanceFinancieroTrasladoValor.tipoTrasladoCodigo ===
                              String(this.tipoTrasladoCodigo.direccionFinanciera)
                        );

                        if (balanceFinancieroTrasladoValor !== undefined) {
                          valorTraslado = balanceFinancieroTrasladoValor.valorTraslado;
                          balanceFinancieroTrasladoValorId =
                    ...
                    const nombreAportante = dataAportantes.listaNombreAportante.find(
                      nombre => nombre.cofinanciacionAportanteId === terceroCausacionDescuento.aportanteId
                    );
                    let listaFuenteRecursos: any[];
                    let fuente: any;
                    if (nombreAportante !== undefined) {
                      listaFuenteRecursos = await this.ordenGiroSvc
                        .getFuentesDeRecursosPorAportanteId(nombreAportante.cofinanciacionAportanteId)
                        .toPromise();
                      fuente = listaFuenteRecursos.find(
                        fuente => fuente.codigo === terceroCausacionDescuento.fuenteRecursosCodigo
                      );
                    }
                    ...
                    listaAportanteDescuentos.push(
                      this.fb.group({
                        registroCompleto: [valorTraslado !== undefined ? true : false],
                        ordenGiroDetalleTerceroCausacionDescuentoId: [
                          terceroCausacionDescuento.ordenGiroDetalleTerceroCausacionDescuentoId
                        ],
                        balanceFinancieroTrasladoValorId: [balanceFinancieroTrasladoValorId],
                        tipoTrasladoCodigo: [String(this.tipoTrasladoCodigo.direccionFinanciera)],
              ...
              terceroCausacion.ordenGiroDetalleTerceroCausacionDescuento.forEach(descuento => {
                if (descuento.tipoDescuentoCodigo !== undefined) {
                  const descuentoIndex = listDescuento.findIndex(
                    descuentoIndex => descuentoIndex.codigo === descuento.tipoDescuentoCodigo
                  );
            ...
            if (terceroCausacion.ordenGiroDetalleTerceroCausacionAportante.length > 0) {
              for (const aportante of terceroCausacion.ordenGiroDetalleTerceroCausacionAportante) {
                const nombreAportante = dataAportantes.listaNombreAportante.find(
                  nombre => nombre.cofinanciacionAportanteId === aportante.aportanteId
                );

                if (nombreAportante !== undefined) {
                  const tipoAportante = dataAportantes.listaTipoAportante.find(
                    tipo => tipo.dominioId === nombreAportante.tipoAportanteId
                  );
                  const tipoAportanteIndex = dataAportantes.listaTipoAportante.findIndex(
                    tipo => tipo.dominioId === nombreAportante.tipoAportanteId
                  );
                  let listaFuenteRecursos: any[] = await this.ordenGiroSvc
                    .getFuentesDeRecursosPorAportanteId(nombreAportante.cofinanciacionAportanteId)
                    .toPromise();
                  ...
                  if (this.balanceFinancieroTrasladoValor !== undefined) {
                    if (this.balanceFinancieroTrasladoValor.length > 0) {
                      const balanceFinancieroTrasladoValor = this.balanceFinancieroTrasladoValor.find(
                        balanceFinancieroTrasladoValor =>
                          balanceFinancieroTrasladoValor.ordenGiroDetalleTerceroCausacionAportanteId ===
                            aportante.ordenGiroDetalleTerceroCausacionAportanteId &&
                          balanceFinancieroTrasladoValor.tipoTrasladoCodigo ===
                            String(this.tipoTrasladoCodigo.aportante)
                      );
                  ...
                  listaAportantes.push(
                    this.fb.group({
                      registroCompleto: [valorTraslado !== undefined ? true : false],
                      ordenGiroDetalleTerceroCausacionAportanteId: [
                        aportante.ordenGiroDetalleTerceroCausacionAportanteId
                      ],
                      balanceFinancieroTrasladoValorId: [balanceFinancieroTrasladoValorId],
              ...
              if (solicitudPagoFaseFactura.solicitudPagoFaseFacturaDescuento !== undefined) {
                if (solicitudPagoFaseFactura.solicitudPagoFaseFacturaDescuento.length > 0) {
                  solicitudPagoFaseFactura.solicitudPagoFaseFacturaDescuento.forEach(descuentoTecnica => {
                    const descuentoTecnicaOrdenGiro = detalleDescuentoTecnica.find(
                      detalleDescuentoTecnica =>
                        detalleDescuentoTecnica.solicitudPagoFaseFacturaDescuentoId ===
                          descuentoTecnica.solicitudPagoFaseFacturaDescuentoId &&
                        detalleDescuentoTecnica.esPreconstruccion === solicitudPagoFase.esPreconstruccion
                    );
                    ...
                    if (descuentoTecnicaOrdenGiro !== undefined) {
                      ...
                      if (this.balanceFinancieroTrasladoValor !== undefined) {
                        if (this.balanceFinancieroTrasladoValor.length > 0) {
                          const balanceFinancieroTrasladoValor = this.balanceFinancieroTrasladoValor.find(
                            balanceFinancieroTrasladoValor =>
                              balanceFinancieroTrasladoValor.ordenGiroDetalleDescuentoTecnicaId ===
                                descuentoTecnicaOrdenGiro.ordenGiroDetalleDescuentoTecnicaId &&
                              balanceFinancieroTrasladoValor.tipoTrasladoCodigo ===
                                String(this.tipoTrasladoCodigo.direccionTecnica)
                          );
                      ...
                      listaDescuentoTecnica.push(
                        this.fb.group({
                          ordenGiroDetalleDescuentoTecnicaId: [
                            descuentoTecnicaOrdenGiro.ordenGiroDetalleDescuentoTecnicaId
                          ],
              ...
              conceptosDePago.push(
                this.fb.group({
                  ...
                  descuento: this.fb.group({
                    ...
                    numeroDescuentos: [
                      terceroCausacion.ordenGiroDetalleTerceroCausacionDescuento.length > 0
                        ? terceroCausacion.ordenGiroDetalleTerceroCausacionDescuento.length
                        : null,
                      Validators.required
                    ],
                  ...
                  descuentoTecnica: this.fb.group({
                    ...
                    numeroDescuentos: [
                      solicitudPagoFaseFactura.solicitudPagoFaseFacturaDescuento.length > 0
                        ? solicitudPagoFaseFactura.solicitudPagoFaseFacturaDescuento.length
                        : null,
                      Validators.required
                    ],
      ...
      const sinDiligenciarCriterio = criteriosArray.find(
        descuento => descuento.get('semaforo').value === 'sin-diligenciar'
      );
    ...
    const sinDiligenciarfase = this.terceroCausacion.controls.find(
      descuento => descuento.get('semaforo').value === 'sin-diligenciar'
    );
    const enProcesofase = this.terceroCausacion.controls.find(
      descuento => descuento.get('semaforo').value === 'en-proceso'
    );
    const completofase = this.terceroCausacion.controls.find(
      descuento => descuento.get('semaforo').value === 'completo'
    );
    ...
    if (value !== null) {
      if (value < 0) {
        ...
        if (tipoValor === 'descuentoFinanciera') {
          this.getAportanteDescuentos(index, jIndex, kIndex, lIndex)
            .controls[mIndex].get('nuevoValorDescuento')
            .setValue(null);
        }
  ...
  guardar(index: number) {
        ...
        if (criterioControl.dirty === true) {
          this.getConceptos(index, indexCriterio).controls.forEach((conceptoControl, indexConcepto) => {
            this.getAportantes(index, indexCriterio, indexConcepto).controls.forEach(aportanteControl => {
              if (aportanteControl.dirty === true) {
                balanceFinancieroTraslado.push({
                  ...
                  ordenGiroDetalleTerceroCausacionAportanteId: aportanteControl.get(
                    'ordenGiroDetalleTerceroCausacionAportanteId'
                  ).value,
            ...
            if (this.getDescuentosFinanciera(index, indexCriterio, indexConcepto).length > 0) {
              this.getDescuentosFinanciera(index, indexCriterio, indexConcepto).controls.forEach(
                (descuentoFinanciera, indexDescFinanciera) => {
                  this.getAportanteDescuentos(
                    index,
                    indexCriterio,
                    indexConcepto,
                    indexDescFinanciera
                  ).controls.forEach(aportanteDescControl => {
                    if (aportanteDescControl.dirty === true) {
                      balanceFinancieroTraslado.push({
                        balanceFinancieroTrasladoId: this.balanceFinancieroTrasladoId,
                        esPreconstruccion: this.terceroCausacion.controls[index].get('esPreconstruccion').value,
                        balanceFinancieroTrasladoValorId: aportanteDescControl.get('balanceFinancieroTrasladoValorId')
                          .value,
                        tipoTrasladoCodigo: aportanteDescControl.get('tipoTrasladoCodigo').value,
                        ordenGiroDetalleTerceroCausacionDescuentoId: aportanteDescControl.get(
                          'ordenGiroDetalleTerceroCausacionDescuentoId'
                        ).value,
    ...
    this.balanceSvc.createEditBalanceFinanciero(this.balanceFinanciero).subscribe(
      response => {
