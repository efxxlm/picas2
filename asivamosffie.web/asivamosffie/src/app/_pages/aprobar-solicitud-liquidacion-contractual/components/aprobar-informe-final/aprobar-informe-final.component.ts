...
export class AprobarInformeFinalComponent implements OnInit {
  ...
  getInformeFinalByProyectoId(proyectoId: number, contratacionId: number) {
    this.registerContractualLiquidationRequestService.getInformeFinalByProyectoId(proyectoId, contratacionId, this.listaMenu.aprobarSolicitudLiquidacionContratacion).subscribe(report => {
      if(report != null){
