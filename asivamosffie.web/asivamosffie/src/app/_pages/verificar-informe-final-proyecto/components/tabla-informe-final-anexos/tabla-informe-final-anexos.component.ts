...
} from 'src/app/_interfaces/informe-final';
...
export class TablaInformeFinalAnexosComponent implements OnInit, AfterViewInit {
  ...
  getInformeFinalListaChequeoByInformeFinalId(id: number) {
    this.verificarInformeFinalService.getInformeFinalListaChequeoByInformeFinalId(id).subscribe(listChequeo => {
      if (listChequeo != null) {
  ...
  openDialogObservaciones(informe: any, verDetalle: boolean) {
    ...
    this.dataSource.data.forEach(control => {
      if (informe !== null && informe.informeFinalInterventoriaId === control.informeFinalInterventoriaId) {
        if (
          control.informeFinalInterventoriaObservaciones != null &&
          control.informeFinalInterventoriaObservaciones.length > 0
        ) {
  ...
  updateStateValidateInformeFinalInterventoriaByInformeFinal(informeFinal: any, test: boolean) {
    this.verificarInformeFinalService
      .updateStateValidateInformeFinalInterventoriaByInformeFinal(informeFinal)
      .subscribe(
        (respuesta: Respuesta) => {
          if (!test) {
