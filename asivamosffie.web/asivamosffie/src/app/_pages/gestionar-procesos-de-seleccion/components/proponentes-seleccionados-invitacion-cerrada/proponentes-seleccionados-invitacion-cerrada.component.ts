...
export class FormDatosProponentesSeleccionadosInvitacionCerradaComponent implements OnInit {
  ...
  validaMinimo3() {
    if (
      this.addressForm.get('cuantosProponentes').value != '' &&
      this.addressForm.get('cuantosProponentes').value != null &&
      this.addressForm.get('cuantosProponentes').value != undefined &&
      this.addressForm.get('cuantosProponentes').value < 3
    ) {
      ...
      forkJoin([
        ...
        this.procesoSeleccion.procesoSeleccionProponente.forEach(p => {
          if (p.localizacionIdMunicipio)
            this.commonService
              .listaMunicipiosByIdDepartamento(p.localizacionIdMunicipio.substring(0, 5))
              .subscribe(municipios => {
                let nombreMunicipio = municipios.find(m => m.localizacionId == p.localizacionIdMunicipio);
                let nombreDepartamento = this.listaDepartamentos.find(
                  d => d.localizacionId == p.localizacionIdMunicipio.substring(0, 5)
                );
  ...
  initForm() {
    this.addressForm = this.fb.group({
      cuantosProponentes: [
        this.procesoSeleccion.cantidadProponentesInvitados,
      ...
      direccionPrincipalRepresentanteLegal: [
        null,
        Validators.compose([Validators.required, Validators.minLength(1), Validators.maxLength(100)])
      ],
  ...
  changeProponente($event: any) {
    ...
    if (this.addressForm.get('cuantosProponentes').value > 0) {
      if (
        ...
        this.addressForm.get('nombresProponentes').value.length
      ) {
        this.procesoSeleccion.procesoSeleccionProponente = [];
      }
      if (
        ...
        this.openDialog(
          '',
          '<b>Ya completo los ' + this.addressForm.get('cuantosProponentes').value + ' proponentes indicados.</b>'
        );
  ...
  cargarRegistro() {
    this.ngOnInit().then(() => {
      ...
      this.procesoSeleccion.procesoSeleccionProponente.forEach(element => {
        ...
        // console.log(this.nombresProponentesList);
        let nombre = this.nombresProponentesList.filter(
          x => x.procesoSeleccionProponenteId == element.procesoSeleccionProponenteId
        );
        if (nombre.length > 0) {
