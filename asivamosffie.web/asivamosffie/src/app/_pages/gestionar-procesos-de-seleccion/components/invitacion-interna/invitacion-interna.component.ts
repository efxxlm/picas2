...
@Component({
  selector: 'app-invitacion-interna',
  templateUrl: './invitacion-interna.component.html',
  styleUrls: ['./invitacion-interna.component.scss']
})
export class InvitacionInternaComponent implements OnInit {
...
ngOnInit() {
  this.activatedRoute.params.subscribe( async parametro => {
    ...
    this.procesoSeleccion.tipoProcesoCodigo = this.tiposProcesoSeleccion.Interna;
...
openDialog(modalTitle: string, modalText: string,id:number) {
  ...
  dialogRef.afterClosed().subscribe(result => {
    this.router.navigate(["/seleccion/invitacionInterna", id]);
      setTimeout(() => {
        location.reload();  
      }, 1000);
  });
