...
export class RegistrarSeguimientoCronogramaComponent implements OnInit {
    ...
    this.activatedRoute.params.subscribe(parametro => {
      ...
      this.procesoSeleccionService.listaActividadesByIdProcesoSeleccion(this.idProcesoSeleccion).subscribe(lista => {
        ...
        lista.forEach(cronograma => {
          ...
          const etapaActualproceso = this.listaTipoIntervencion.find(
          ...
          this.pasado[i] = false;
          if (hoy >= fechaSesion) {
            //si la fecha maxima es mayor a hoy etonces se muestra la ultima opción de la lista "Sin reporte"
          ...
          grupo
            .get('fechaMaxima')
            .setValue(`${fechaSesion.getDate()}/${fechaSesion.getMonth() + 1}/${fechaSesion.getFullYear()}`);
          ...
          if (cronograma.cronogramaSeguimiento.length > 0) {
            const estadoActividad = this.listaEstadosSeguimiento.find(
              p =>
                p.codigo ===
                cronograma.cronogramaSeguimiento[cronograma.cronogramaSeguimiento.length - 1].estadoActividadFinalCodigo
            );
            grupo
              .get('fechaMonitoreo')
              .setValue(cronograma.cronogramaSeguimiento[cronograma.cronogramaSeguimiento.length - 1].fechaCreacion);
            grupo.get('estadoActividad').setValue(estadoActividad);
            grupo
              .get('observacion')
              .setValue(cronograma.cronogramaSeguimiento[cronograma.cronogramaSeguimiento.length - 1].observacion);
            this.estaEditando = true;
            this.addressForm.markAllAsTouched();
            grupo.markAllAsTouched();
            if (
              cronograma.cronogramaSeguimiento[cronograma.cronogramaSeguimiento.length - 1]
                .estadoActividadFinalCodigo == '3'
            ) {
              //si es finalizada, perdon por el "3" tengo sueño
              this.activo[i] = false;
            }
          }
  ...
  onSubmit() {
    ...
    from(this.listaCronograma)
      .pipe(
        mergeMap(cronograma => this.procesoSeleccionService.createEditarCronogramaSeguimiento(cronograma).pipe(tap())),
        toArray()
      )
      .subscribe(
        respuesta => {
  ...
  onChangeEstado() {
    ...
    let listaActividades = this.addressForm.get('actividades') as FormArray;
    let lista = this.listaCronogramaActividades.filter(
      x => x.etapaActualProcesoCodigo == this.addressForm.value.tipoIntervencion.codigo
    );
    lista.forEach(cronograma => {
      let grupo = this.createActividad();
      ...
      //grupo.get('descripcion').setValue( cronograma.descripcion );
      grupo.get('descripcion').setValue(cronograma.descripcion);
      grupo.get('etapaActualProceso').setValue(etapaActualproceso?.nombre), (this.estaEditando = true);
      this.addressForm.markAllAsTouched();
      listaActividades.push(grupo);
    });
  }
